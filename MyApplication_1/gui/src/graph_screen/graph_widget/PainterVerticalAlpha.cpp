...
#define fastrand(g_seed) ((214013*(uint32_t)(g_seed)+2531011)>>16)
    ...
    do
    {
        ...
        uint8_t p_red = Color::getRedColor(*p);
        uint8_t p_green = Color::getGreenColor(*p);
        uint8_t p_blue = Color::getBlueColor(*p);
...
#define fastrand(g_seed) ((g_seed * 1103515245U + 12345U) & 0x7fffffffU)
        uint8_t rand = fastrand((currentX * currentY + *p) * (currentX + currentY));
        if (pixelRed < 0xC0)
        {
            pixelRed = pixelRed + ((rand >> 0) & 0x3F);
        }
        if (pixelGreen < 0xC0)
        {
            pixelGreen = pixelGreen + ((rand >> 1) & 0x3F);
        }
        if (pixelBlue < 0xC0)
        {
            pixelBlue = pixelBlue + ((rand >> 2) & 0x3F);
        }
        *p++ = (uint8_t)Color::getColorFrom24BitRGB(pixelRed, pixelGreen, pixelBlue);
        currentX++;
    }
    while (--count != 0);
#elif USE_BPP==4
